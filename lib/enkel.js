...
module.exports = class {
  ...
  createServer () {
    ...
    console.log('\x1B[34m%s\x1B[39m', '服务已经启动', `http://127.0.0.1:${enkel._caches.configs.app_port}`);
  }
  ...
  createSocketServer () {
    ...
    let app = null
    if (_socketConfig.type === 'wss') {
      const https = require('https');
      let keypath = _socketConfig.key || path.resolve(process.cwd(), '../../../etc/letsencrypt/live/dei2.com-0001/privkey.pem');
      let certpath = _socketConfig.cert || path.resolve(process.cwd(), '../../../etc/letsencrypt/live/dei2.com-0001/cert.pem');
      let sslOptions = {
        key: fs.readFileSync(keypath),
        cert: fs.readFileSync(certpath)
      }
      app = https.createServer(sslOptions, function (req, res) {
        res.writeHead(403);
        res.end('');
      })
    } else {
    ...
    console.log('\x1B[34m%s\x1B[39m', 'socket服务已经启动', `http://127.0.0.1:${_socketConfig.port}`);
